...
function autoGenerateDueDate(assignedTime: string): string {
    ...
    // Use UTC-based calculation to avoid timezone shifting the date
        ...
        try {
            ...
            for (const { course, coursework } of classroomData) {
                ...
                for (const work of coursework) {
                    ...
                    // Use updateTime as the "assigned date" — this is when faculty
                    // published/shared the assignment (what students see as the date).
                    // creationTime can be when the draft was first created, which may
                    // be weeks before the actual assignment date.
                    const assignedTime = work.updateTime || work.creationTime;

                    if (newImportedIds.has(courseworkKey)) {
                        // ──── Already imported — fix due date if needed ────
                        // If faculty didn't set a due date, ensure our auto-generated
                        // one is based on updateTime (not creationTime)
                        if (!facultyDueDate) {
                            const correctDueDate = autoGenerateDueDate(assignedTime);
